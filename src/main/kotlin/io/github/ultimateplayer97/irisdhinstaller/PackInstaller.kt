...
class PackInstaller(
    private val packVersion: PackVersion,
    ...
    private fun updateLauncherProfiles() {
        ...
        profile["javaArgs"] =
            "-Xmx$javaArgs -XX:+UnlockExperimentalVMOptions -XX:+UseG1GC -XX:G1NewSizePercent=20 -XX:G1ReservePercent=20 -XX:MaxGCPauseMillis=50 -XX:G1HeapRegionSize=32M"
    ...
    private fun installLoader() {
        val gameVersion = packVersion.gameVersion
            ?: throw IllegalStateException("Loader version not found for ${packVersion.loader.dependencyName}")

        progressHandler.newTaskSet(8)

        val loaderVersionElement = packIndex["dependencies"].asJsonObject[packVersion.loader.dependencyName]
        val loaderVersion = loaderVersionElement?.asString
            ?: throw IllegalStateException("Loader version not found for ${packVersion.loader.dependencyName}")
